...
```{r}
json_data <- fromJSON("newman-run-report-2020-06-25-06-42-13-124-0-ticketid-upto-3000.json")      # Display json file
str(json_data) # Show where the content is
```
...
for (json_file in file_list) {
  ...
  for (event in json_data) {
    cat(sprintf("prefix = %s, event = %s\n", event[['prefix']], event[['event']]))
  }
  rm(json_data) # Free up memory
}
```
